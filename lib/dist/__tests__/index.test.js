...
beforeEach(() => {
    ...
    utilMock.promisify.mockImplementation((_fn) => {
        return jest.fn().mockImplementation((command) => {
            // Respuestas específicas según el comando
            if (typeof command === 'string') {
                if (command.includes('docker-compose ps')) {
...
describe('BesuNetwork', () => {
    describe('Inicialización', () => {
        test('debería inicializar con valores por defecto', () => {
            const network = new __1.BesuNetwork();
        ...
        test('debería inicializar con valores personalizados', () => {
            const network = new __1.BesuNetwork('red-personalizada', { image: 'besu:custom' });
    ...
    describe('Configuración de la red', () => {
        test('debería guardar la configuración de la red correctamente', () => {
            const network = new __1.BesuNetwork('test-network');
            ...
            expect(fs.writeFileSync).toHaveBeenCalledWith('/test/output/node-1/key', expect.any(String));
            expect(fs.writeFileSync).toHaveBeenCalledWith('/test/output/node-2/key', expect.any(String));
            // Comprobar direcciones
            expect(fs.writeFileSync).toHaveBeenCalledWith('/test/output/node-1/address', expect.any(String));
            expect(fs.writeFileSync).toHaveBeenCalledWith('/test/output/node-2/address', expect.any(String));
        });
        test('debería generar un archivo genesis.json válido', () => {
            ...
            const writeCall = fs.writeFileSync.mock.calls.find(call => call[0] === '/test/output/genesis.json');
            if (writeCall) {
        ...
        test('debería generar configuración Besu para cada nodo', () => {
            ...
            expect(fs.writeFileSync).toHaveBeenCalledWith('/test/output/node-2/config.toml', expect.any(String));
            // Verificar contenido
        ...
        test('debería generar docker-compose.yml para la red', () => {
            ...
            // Verificar contenido
            const writeCall = fs.writeFileSync.mock.calls.find(call => call[0] === '/test/output/docker-compose.yml');
            if (writeCall) {
    ...
    describe('Operaciones con Docker', () => {
        test('debería iniciar la red correctamente', () => __awaiter(void 0, void 0, void 0, function* () {
            const network = new __1.BesuNetwork();
            network.createNode({ name: 'node-1' });
            network.createNode({ name: 'node-2' });
            ...
            network.startNetwork = jest.fn().mockImplementation(() => __awaiter(void 0, void 0, void 0, function* () {
            ...
            yield network.startNetwork('/test/output');
        ...
        test('debería detener la red correctamente', () => __awaiter(void 0, void 0, void 0, function* () {
            const network = new __1.BesuNetwork();
            ...
            yield network.stopNetwork('/test/output');
        ...
        test('debería eliminar la red correctamente', () => __awaiter(void 0, void 0, void 0, function* () {
            const network = new __1.BesuNetwork();
            network.createNode({ name: 'node-1' });
            yield network.destroyNetwork('/test/output');
            // Verificar que se eliminó el directorio
            expect(fs.rmSync).toHaveBeenCalledWith('/test/output', { recursive: true, force: true });
        }));
        test('debería obtener el estado de la red correctamente', () => __awaiter(void 0, void 0, void 0, function* () {
            const network = new __1.BesuNetwork();
            network.createNode({ name: 'node-1' });
            network.createNode({ name: 'node-2' });
            ...
            const execAsyncMock = jest.fn().mockImplementation((command) => {
            ...
            network.getNetworkStatus = jest.fn().mockImplementation(() => __awaiter(void 0, void 0, void 0, function* () {
                return {
                    running: true,
                    nodes: {
                        'node-1': {
            ...
            const status = yield network.getNetworkStatus('/test/output');
            // Verificar que el método mock fue llamado
        ...
        test('debería reiniciar la red correctamente', () => __awaiter(void 0, void 0, void 0, function* () {
            const network = new __1.BesuNetwork();
            network.createNode({ name: 'node-1' });
            ...
            yield network.restartNetwork('/test/output');
    ...
    describe('Despliegue de red', () => {
        test('debería desplegar una red completa', () => __awaiter(void 0, void 0, void 0, function* () {
            const network = new __1.BesuNetwork();
            ...
            yield network.deployNetwork(3, 2, '/test/output');
        ...
        test('debería validar que numSigners <= numNodes', () => __awaiter(void 0, void 0, void 0, function* () {
            const network = new __1.BesuNetwork();
            yield expect(network.deployNetwork(2, 3, '/test/output')).rejects.toThrow();
        }));
    ...
    describe('Carga desde configuración', () => {
        test('debería cargar una red desde un archivo de configuración', () => {
            const network = __1.BesuNetwork.loadFromConfig('/test/network-config.json');
        ...
        test('debería lanzar error si el archivo no existe', () => {
            fs.existsSync.mockReturnValue(false);
            expect(() => __1.BesuNetwork.loadFromConfig('/nonexistent/config.json')).toThrow();
        });
    });
