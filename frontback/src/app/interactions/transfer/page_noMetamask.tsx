...
export default function Transfer() {
    ...
    const handleTransfer = async (e: React.FormEvent) => {
        ...
        if (!/^(0x)?[0-9a-fA-F]{40}$/.test(fromAddress) || !/^(0x)?[0-9a-fA-F]{40}$/.test(toAddress)) {
        ...
        try {
            const response = await fetch('/api', {
                ...
                body: JSON.stringify({ from: fromAddress, to: toAddress, amount, network: selectedNetwork }),
            });
            ...
            setError('An error occurred while transferring.');
