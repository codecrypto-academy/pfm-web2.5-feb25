...
You only have to launch the script to create the network:

```bash
./script.sh
```

And the `script` will create a Besu (Clique) three nodes network following the next steps:
...
echo "-------------------------------------------"
```
### Delete all previous network data and Docker containers
```bash
echo ""
